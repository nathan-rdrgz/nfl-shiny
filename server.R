shiny::shinyServer(function(session, input, output) {
  ...
  filtered_data <- reactive({
    ...
    offPenalties <- input$offensive_penalties
    defPenalties <- input$defensive_penatlies
    ...
    if (offPenalties) DT2 <- DT2[offPenalty != 1]
    if (defPenalties) DT2 <- DT2[defPenalty != 1]
  ...
  observeEvent(
    input$getOptimal, 
    {
      ...
      newLOC <- 'data/'
      newCSV <- 'FanDuel-NFL-2021-01-03-52913-players-list.csv'
